...
Status: auth, check-in/out (with half-day, location, late/early flags, a hard permission gate on checking in, and a per-employee "roaming" exemption from auto-checkout), attendance history (paginated, day-grouped, tap-through detail views), weekly schedule, tasks (assign/edit/delete, filterable, now via a "+"-button-opens-modal-route form instead of an always-visible inline one), leave requests (with balance, filterable, optional photo/location attachment), locations management (same modal-route form pattern), profile pictures, employee profile editing, staff search, a redesigned compact header with the real company logo, and an admin dashboard are all built. Phase 1 was verified against a real Supabase project as of 2026-09-05; as of 2026-09-13/14, a real emulator became available in-session for the first time and a large amount of subsequent work got verified live (see `TODO.md`'s Quick Status for exactly what) — some more granular pieces from the 2026-09-10 batch, and a couple of employee-only flows this session lacked credentials for, remain unverified. Geofencing auto-checkout (Phase 2) is code-complete but **the actual real-device exit-triggers-auto-checkout behavior is still not verified** — see `TODO.md`. KPI and notifications are not built yet. A separate sibling project, `../attendance-download`, hosts the Android/iOS install page (see `ARCHITECTURE.md`'s Distribution section).
